...
Grafo::~Grafo(){std::cout << "algo3.5.1" <<std::endl;
	this->grafo_anular();
}
...
void Grafo::grafo_inserta_vertice(int _i_id,int _i_posx, int _i_posy, Grafo *_i_grafo_lod1){
	Vertice *_nuevo = new Vertice;
	_nuevo->_id=_i_id;
	_nuevo->_sig=nullptr;
	_nuevo->_ady=nullptr;
	_nuevo->_lod1=_i_grafo_lod1;
	_nuevo->_posx=_i_posx;
	_nuevo->_posy=_i_posy;
...
void Grafo::grafo_anular(){
	...
	while(_h!=nullptr){
		std::cout<< "_aux" << _aux->_id << std::endl;
		_aux=_h;
std::cout<< "_aux" << _aux<< std::endl;
