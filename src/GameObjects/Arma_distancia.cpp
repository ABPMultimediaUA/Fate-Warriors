...
Arma_distancia::Arma_distancia(float _i_x, float _i_y, float _i_z, float _i_ancho, float _i_alto, short _i_uses) : Equipable(_i_x, _i_y, _i_z,_i_ancho, _i_alto, _i_uses){
...
bool Arma_distancia::usar(){
	std::cout << _uses<< "/" << _balas_totales << std::endl;
	if (decrease_uses()){
		/*
		std::cout << Balas.size();
