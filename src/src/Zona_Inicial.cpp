...
bool Zona_Inicial::hay_personajes_conquistando_esta_zona(){
...
void Zona_Inicial::set_tiempo_restante_para_conquistar(){
...
void Zona_Inicial::cambiar_bando(){
    _equipo=personajes_de_la_zona[0]->get_equipo();
    _conquistando=false;
}

//Restaura el tiempo de conquista
void Zona_Inicial::iniciar_tiempo_conquista(){
    _tiempo_inicio_conquista = _reloj->get_current();
}

std::vector <Character*> Zona_Inicial::get_characters(){
    return personajes_de_la_zona;
}

bool Zona_Inicial::get_conquistando(){
    return _conquistando;
}

void Zona_Inicial::set_interruptor_asociado(Interruptor* _i_interruptor){
    _interruptores_asociados.push_back(_i_interruptor);
}

std::vector<Interruptor*> Zona_Inicial::get_interruptores_asociados(){
