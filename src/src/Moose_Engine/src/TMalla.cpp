...
void TMalla::draw(Shader* _i_shader){
...
void TMalla::cargarModelo(std::string &path){
...
void TMalla::cargarNodo(aiNode* nodo, const aiScene* scene){
    ...
    for(unsigned int i = 0; i < nodo->mNumMeshes; i++){
        ...
        _mallas.push_back(cargarMalla(mesh, scene));
    }
    ...
    for(unsigned int i = 0; i < nodo->mNumChildren; i++){
        cargarNodo(nodo->mChildren[i], scene);
    }
}
TRecursoMalla TMalla::cargarMalla(aiMesh *mesh, const aiScene *scene){
    // data to fill
    ...
    */
    ...
    return TRecursoMalla(vertices, indices, textures);
}