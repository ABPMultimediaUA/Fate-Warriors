...
void Camara::Camara_Update() {
	...
	if (_Prota != nullptr) {
		...
		float xf = playerPos.X - cos(_zdirection * irr::core::PI / 180.0f) * cos(_direction * irr::core::PI / 180.0f) * 500.0f; 
		
		float yf = playerPos.Y - sin(_zdirection * M_PI / 180.0f) * 128.0f; 

		float zf = playerPos.Z + cos(_zdirection * irr::core::PI / 180.0f) * sin(_direction * irr::core::PI / 180.0f) * 500.0f; 
