...
void Camara::Camara_setProta(scene::ISceneNode * prota) {
	_Prota = prota; 
	_ProtaBoundingCenter = _Prota-> getBoundingBox().getCenter().Y; 
}
...
void Camara::Camara_Update() {
	...
	if (_Prota != nullptr) {
		...
		core::vector3df playerPos = core::vector3df(
					...
					_Prota-> getPosition().Y - _ProtaBoundingCenter, 
		...
		//std::cout<<"llego loko"<<std::endl;
			...
			this->Camara_setPosition(
					core::vector3df(xf, 
							yf + _ProtaBoundingCenter, 
							zf)); 
			
			this->Camara_setTarget(	
					core::vector3df(_Prota-> getPosition().X, 
							_Prota-> getPosition().Y - _ProtaBoundingCenter * 1.5f, 
							_Prota-> getPosition().Z)); 
			...
			this->Camara_setPosition(
					core::vector3df(xf, 
							yf - _Prota-> getBoundingBox().getCenter().Y - _Prota->getPosition().Y, zf)); 
			
			this->Camara_setTarget(
					core::vector3df(_Prota-> getPosition().X, 
							(_ProtaBoundingCenter * 0.5), _Prota-> getPosition().Z)); 
		}
